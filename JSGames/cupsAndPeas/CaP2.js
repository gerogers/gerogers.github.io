...
canvas.width = 100;   // initial value
canvas.height = 100;  // initial value
...
let curDir;
let puzNo = 0;
let puzType = 0;   // 0 = cups and peas, 3 = synchronised tilt maze
let moves = 0;
let cols = 1;
let rows = 1;
let scale = 1;
...
function resize() {
    // reduce canvas size to ensure it fits the current orientation
    canvas.width = 100;
    canvas.height = 100;
    let i = 0;
    ...
    for (i = 0; i < dropContent.length; i++) {
        dropContent[i].style.fontSize = `${smlsz}px`;
    }
    let actsContent = document.getElementsByClassName("actions-content");
    for (i = 0; i < actsContent.length; i++) {
        actsContent[i].style.fontSize = `${sz}px`;
    }
    
    puzlId.style.fontSize = `${sz}px`;
    let fs = document.getElementById("msgDrop").style.fontSize;
    let fnt = document.getElementById("msgDrop").style.font;
    //alert(`fontSize ${fs}, font ${fnt}`);
    document.getElementById("msgDrop").style.fontSize = `${smlsz}px`;
...
function reset() {
    ...
    if (cols > 1) {
        ...
        for (let i = 0; i < elements; i++) {
            tile = dcd(params[puzNo].value, table + i);
            // relocate colour of pea to allow peas to enter cups
            // or colour of sliding tile to allow tiles to cover targets
            if ((tile & 8) != 0) {
                // this is a pea/slider, move its colour field
    ...
    drawPuzl();
    document.getElementById("actionsDrop").classList.remove("show");
}
        ...
        for (let y = 0; y < rows; y++) {
            ...
            if ((tile & 4) != 0) {
                ...
                if (puzType == 0) {    // cups and peas
                    ...
                    switch(dir) {
                        case N:
                            if (!rot) {
                                ctx.fillRect(cupX - 5 * scale,
                                    cupY - 11 * scale, 10 * scale, 11 * scale);
                            }
                            break;
                        case E:
                            if (rot) {
                        ...
                        case W:
                            if (rot) {
                                ...
                                ctx.fillRect(cupX - 11 * scale,
                                    cupY - 5 * scale, 11 * scale, 10 * scale);
                            }
                            break;
                    }
                } else if (puzType == 3) {    // synchronised tilt maze
                    ctx.fillRect(cupX - 6 * scale, cupY - 6 * scale,
                                    12 * scale, 12 * scale);
                }
            ...
            if ((tile & 8) != 0) {
                ...
                if ((tile & 0x2000) != 0) {
                    ...
                    switch(peaDir) {
                    case N:
                        // the co-ord represents the initial position,
                        // so show the pea further north
                        cupY -= animStep * scale;
                        break;
                    case E:
                        cupX += animStep * scale;
...
function puzl(puz) {
    puzNo = puz;
    puzlId.innerHTML = params[puzNo].title;

    let cr;
    let b0 = dcd(params[puzNo].value, 0);
    if (b0 < 16) {
        // zero cols means extended header, cols/rows comes later (b0 later)
        puzType = dcd(params[puzNo].value, b0);
        table = 2 + b0;
        cr = dcd(params[puzNo].value, 1 + b0);
    } else {
        puzType = 0;
        table = 1;
        cr = b0;
    ...
    for (let i = 0; i < rows * cols; i++) {
        ...
        if ((tile & 4) != 0) {
            ...
            if (cupClr != 0) {
                ...
                let peaClr = (tile & 0x1C00) >> 10;
                if (cupClr != peaClr) {
                    // alert(`tile ${cupClr} pea ${peaClr}`); 
            ...
            for (x = 0; x < cols; x++) {
                ...
                if ((tile & 8) != 0) {
                    ...
                    if (((puzType == 0) &&
                        ...
                        ((puzType == 3) && ((tile2 & 0x2) == 0))) {
                        // no obstruction, start moving pea
                        // mark this piece as moving and record its direction
                        tile &= ~0xC000;    // clear MovDir
                        tile |= ((dir << 14) | 0x2000);
                        playArea[i] = tile;
                        moving = true;
                    }
            ...
            for (y = 0; y < rows; y++) {
                ...
                if ((tile & 8) != 0) {
                    ...
                    if (((puzType == 0) &&
                        ...
                        // no obstruction, start moving pea
                        // mark this piece as moving and record its direction
                        tile &= ~0xC000;   // clear MovDir field
                        tile |= (dir << 14) | 0x2000;
                        playArea[i] = tile;
                        moving = true;
                    }
            ...
            for (y = 0; y < rows; y++) {
                ...
                tile = playArea[i];
                    ...
                    if (((puzType == 0) &&
                        ...
                        ((puzType == 3) && ((tile2 & 1) == 0))) {
                        // no obstruction, move start moving pea
                        // mark this piece as moving and record its direction
                        tile &= ~0xC000;    // clear MovDir field
                        tile |= (dir << 14) | 0x2000;
                        playArea[i] = tile;
...
function animComplete() {
    ...
    if (rotating) {
        ...
        // record new position of all moving peas and clear moving flag
        for (let i = 0; i < rows * cols; i++) {
            tile = playArea[i];
            if ((tile & 8) != 0) {
                if ((tile & 0x2000) != 0) {
                    // this pea has just finished moving, relocate it
                    let j;
                    switch ((tile & 0xC000) >> 14) {
                    case N:
                        j = i - 1;
                        break;
                    case E:
                    ...
                    if (((tile2 & 8) != 0) && (puzType == 3)) {
                        ...
                        peaClr = tile & 0x1C08;    // save pea colour & Pea
                        tile  &= ~0xFC08;    // clear MovDir MVP PeaClr Pea
                        tile2 &= ~0xFC08;    // clear MovDir MVP PeaClr Pea
                        // restore pea attributes to new location
                        tile2 |= peaClr;
                        playArea[i] = tile;
                        playArea[j] = tile2;
                    }
